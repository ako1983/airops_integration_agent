...
from langchain_anthropic import ChatAnthropic  # Use specialized package
...
class AgentState(TypedDict):
    """State for the integration agent."""
    user_request: str
    context: Dict[str, Any]
    integration_actions: List[Dict[str, Any]]
    query_refiner_output: Optional[RefinedQuery]
    planner_output: Optional[Dict[str, Any]]
    schema_retriever_output: Optional[Dict[str, Any]]
    parameter_generator_output: Optional[Dict[str, Any]]
    validator_output: Optional[Dict[str, Any]]
    repair_output: Optional[Dict[str, Any]]
    workflow_generator_output: Optional[Dict[str, Any]]
...
def create_enhanced_integration_agent():
    ...
    try:
        ...
        # Fallback to a mock LLM if needed
        from langchain.llms.fake import FakeListLLM
        llm = FakeListLLM(responses=["This is a fallback response"])

    # Initialize state graph with proper schema
